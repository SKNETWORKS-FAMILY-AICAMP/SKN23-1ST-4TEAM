...
[주소기반산업지원서비스](https://business.juso.go.kr/addrlink/elctrnMapProvd/geoDBDwldList.do?menu=%EA%B5%AC%EC%97%AD%EC%9D%98%20%EB%8F%84%ED%98%95)에서 제공하는 구역의 도형 데이터(.shp)를 geoJson으로 변환하여 Plotly차트로 광역시/도 지도 구현
...
[노션 참고](https://www.notion.so/2c15b4cf83ab8053b1c4ffa904342e6b?v=2c15b4cf83ab80eebddb000ca1f45b53&source=copy_link)
...
- **팀원 작업 검토 및 소통 시간 지연**
  ...
  - 문제점: 팀원들의 작업 결과물을 검토하고 피드백을 전달하는 과정에서 예상보다 많은 시간이 소요되었습니다. 특히 Git 사용에 미숙한 팀원들이 있어 코드 병합(Merge) 및 업데이트(Pull) 과정에서 충돌이 자주 발생하여 추가적인 개입이 필요했습니다.
  - 배운 점: 프로젝트 초반에 Git 사용법에 대한 충분한 교육과 명확한 코드 리뷰 프로세스 확립의 중요성을 깨달았습니다. 초기 투자 시간을 통해 전체 프로젝트 진행 속도와 효율성을 높일 수 있음을 확인했습니다.

- **초기 작업량 분배 및 역할 조정의 어려움**

  - 문제점: 첫 기획 회의 시 각 팀원의 실력과 예상 작업 시간을 고려하여 역할을 분배했으나, 실제 프로젝트 진행 중 일부 팀원이 작업을 기한 내 완료하지 못해 다른 팀원이 대신 처리하는 상황이 발생했습니다. 이는 초기 작업량 분배 계획이 실제 역량을 충분히 반영하지 못했음을 보여줍니다.
  - 배운 점: 개인의 역량과 성장 속도를 면밀히 파악하여 작업량을 분배하고, 프로젝트 진행 중에도 유연하게 역할과 작업량을 조정할 수 있는 체계가 필요함을 절감했습니다. 팀원 개개인의 성장 또한 중요한 요소로 고려해야 합니다.

- **압박감 속에서 효과적인 팀 커뮤니케이션 유지의 어려움**

  - 문제점: 마감 기한이 임박하거나 예상치 못한 오류가 발생했을 때, 팀원들의 실수나 지연에 대해 저를 포함한 팀 전체가 압박감을 느끼고 예민하게 반응하는 모습을 보였습니다. 특히 발표 직전까지 오류가 지속되어 멘탈 관리에 실패하고, 팀원들에게 부정적인 감정을 표출하는 좋지 못한 모습을 보여 반성하게 되었습니다.
  - 배운 점: 리더는 예상치 못한 문제 발생 시에도 침착하게 상황을 분석하고, 팀원들을 격려하며 함께 해결책을 찾아나가는 긍정적인 리더십의 중요성을 깊이 통감했습니다. 감정적인 대응보다는 명확한 문제 해결 방안 제시와 팀워크를 다지는 것이 중요하다는 것을 배웠습니다.


**💻 Front-end 개발 중**

- **위도, 경도 값을 이용한 지도 차트 구현**

  - 문제점: 한반도의 '광역시/도' 단위로만 구성된 지도를 구현하는 데 어려움을 겪었습니다. 기존의 지도 라이브러리들은 주로 국가 단위나 전 세계 지도를 기본으로 제공하여 국내 광역 단위의 세부적인 데이터 연동이 쉽지 않았습니다. 또한 국내 데이터들중에서는 '광역시/도'보다 더 세분화된 행정동 단위의 데이터들이 대부분이여서 '광역시/도'데이터를 찾는데 어려움이 있었습니다.
  - 해결: 국토교통부에서 지원하는 '광역시/도'별 경도, 위도 값을 GeoJSON 형식으로 변환하여 확보했습니다. 이를 Plotly 차트 라이브러리와 연동하여 지역별 데이터를 시각적으로 표현하는 지도 차트를 성공적으로 구현했습니다.

- **아이콘(FontAwesome) 적용**

  - 문제점: FontAwesome에서 제공하는 CSS 파일, CDN 링크, JavaScript 스크립트 등 다양한 방식을 시도했지만, 아이콘이 정상적으로 인식되지 않거나 클래스명이 일치하지 않아 깨져 보이는 문제가 발생했습니다.
  - 진행 상황: 현재는 보류 중인 상태입니다. 추후 재작업 시에도 동일한 문제가 발생하여 구현이 불가할 경우, 아이콘 이미지 파일을 직접 활용하는 최후의 방법을 고려하고 있습니다.

- **CSS 스타일/테마 적용**

  - 문제점: Streamlit 요소를 마크다운을 이용한 HTML 태그 요소들과 함께 사용하는 과정에서, 일반 CSS로는 Streamlit 요소들의 스타일을 직접 제어하기 어렵다는 점을 발견했습니다.
  - 해결: Streamlit 요소들의 스타일은 .streamlit/config.toml 테마 파일을 활용하여 전역적으로 설정했습니다. HTML 태그 요소들은 Streamlit_app.py 파일 내에서 공동 CSS 스타일을 직접 삽입하여 테마 및 스타일을 일관되게 적용할 수 있었습니다.

- **네비게이션에서 서브메뉴 들여쓰기 표현**

  - 문제점: 네비게이션 메뉴에 서브메뉴를 구현할 때, 좌측에 들여쓰기(margin 또는 padding)를 적용하여 계층 구조를 시각적으로 표현하고 싶었으나, Streamlit의 UI 특성상 직접적인 CSS 제어가 어려워 원하는 만큼의 들여쓰기 구현이 쉽지 않았습니다.
  - 해결: 각 메뉴 항목을 독립적인 st.button()으로 구성한 후, CSS를 직접 주입하여 버튼들의 스타일을 섬세하게 조작했습니다. 서브메뉴의 들여쓰기는 유니코드 공백 기호를 사용하여 시각적인 계층감을 부여하는 방식으로 해결했습니다.
